...
export class ClientService {
  ...
  protected convertDateFromClient(client: IClient): IClient {
    const copy: IClient = Object.assign({}, client, {
      birthDate: client.birthDate && client.birthDate.isValid() ? client.birthDate.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((client: IClient) => {
        client.birthDate = client.birthDate ? moment(client.birthDate) : undefined;
      });
    }
    return res;
  }
